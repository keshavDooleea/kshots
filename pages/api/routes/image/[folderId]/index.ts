...
import { IMAGES_SCHEMA, IMG_SRC } from "../../../../../utils/lib/config";
...
const handleGET = async (req: NextApiRequest, res: NextApiResponse<any>, session: INewSession) => {
  ...
  try {
    ...
    const getQuery = `SELECT id, userid, folderid, ${IMG_SRC} as src, createdat, title, description FROM ${IMAGES_SCHEMA} WHERE userid = $1 AND folderid = $2;`;
